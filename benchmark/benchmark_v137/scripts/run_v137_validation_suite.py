"""Run the v1.37 validation workflow end to end."""
...
RESULT_DIR = REPO_ROOT / "benchmark" / "benchmark_v137" / "results" / "v137_release_candidate"
...
def main() -> None:
    py = sys.executable
    _run([py, "benchmark/benchmark_v136/scripts/run_v136_validation_suite.py"])
    _run([py, "benchmark/benchmark_v137/scripts/build_v137_release_candidate_evidence_pack.py", "--build"])
    _run(
        [
            py,
            "benchmark/benchmark_v137/scripts/validate_v137_release_candidate.py",
            "--result-dir",
    ...
    _run([py, "benchmark/benchmark_v137/scripts/generate_v137_adversarial_cases.py", "--source-result-dir", "benchmark/benchmark_v137/results/v137_release_candidate"])
    _run(
        [
            py,
            "benchmark/benchmark_v137/scripts/validate_v137_release_candidate.py",
    ...
    if injected.get("detected_defects") != injected.get("seeded_defects"):
        raise SystemExit("not every v1.37 injected defect was detected")
    _run([py, "-m", "unittest", "discover", "-s", "benchmark/benchmark_v137/tests"])
    print("v1.37 validation suite PASS")
